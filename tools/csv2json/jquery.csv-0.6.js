...
(function( $ ) {
  ...
  $.csvEntry2Array = function(csv, meta) {
    ...
    csv.replace(reValue, function(m0, m1, m2) {
      // Remove backslash from any delimiters in the value
        if (m2 !== undefined && m2 !== null && m2 !== "") {
            output.push(m2);
        } else if (m1 !== undefined && m1 !== null) {
  ...
  $.csv2Dictionary = function(csv, meta) {
    ...
    for(var i in lines) {
      ...
      var lineDict = {};
      var NonEmptyValuePresent = false;
      for(var j in headers) {
        lineDict[headers[j]] = line[j];
        if (line[j] !== undefined && line[j] !== null && line[j] !== "") {
            NonEmptyValuePresent = true;
        }
      }

      if (NonEmptyValuePresent) {
